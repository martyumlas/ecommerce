        ...
        props: ['attributeid'],
          data() {
            ...
            methods: {
                ...
                saveValue() {
                      if (this.value === '') {
                        this.$swal({title : '"Error, Value for attribute is required."', icon : 'error'});
                    } else {
                        let attributeId = this.attributeid;
                        let _this = this;
                        axios.post('/admin/attributes/add-values', {
                            ...
                            _this.resetValue();
                            _this.$swal({title: "Success! Value added successfully!", 
                                icon: "success",
                            });
                        }).catch(function (error) {
                            console.log(error);
                ...
                deleteAttributeValue(value) {
                    this.$swal({
                        title: "Are you sure?",
                        text: "Once deleted, you will not be able Pto recover this attribute value!",
                        icon: "warning",
                        buttons: true,
                        dangerMode: true,
                    }).then((willDelete) => {
                        if (willDelete) {
                            ...
                            axios.post('/admin/attributes/delete-values', {
                                ...
                                if (response.data.status === 'success') {
                                    _this.values.splice(_this.key, 1);
                                    _this.resetValue();
                                    _this.$swal({title : "Success! Option value has been deleted!", 
                                        icon: "success",
                                    });
                                } else {
